...
do_bootloaders_overlays_fit() {
    ...
    if [[ "$MACHINE" == "rzv2n-solidrun" ]]; then
        echo "Skipping overlays FIT for RZ/V2N (not required)"
        return
    fi
...
do_bootloaders_binman() {
    ...
    # Use RZ/V2N-specific binman config (no overlays)
    local BINMAN_DTS="${ROOTDIR}/configs/image/binman-boot-image.dts"
    if [[ "$MACHINE" == "rzv2n-solidrun" ]]; then
        BINMAN_DTS="${ROOTDIR}/configs/image/binman-boot-image-rzv2n.dts"
    fi
	dtc -I dts -O dtb -o ${BUILDDIR_TMP_BOOT_IMAGE}/${MACHINE}/u-boot.dtb ${BINMAN_DTS}
...
do_bootloaders_install() {
    ...
    if [[ -f "${BUILDDIR_TMP_BOOT_IMAGE}/overlays.itb" ]]; then
